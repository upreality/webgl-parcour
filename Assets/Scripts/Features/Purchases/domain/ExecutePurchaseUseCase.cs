...
namespace Features.Purchases.domain
{
    public class ExecutePurchaseUseCase
    {
        ...
        {
            PurchaseType.Coins => TryCurrencyPurchase(purchaseId),
            PurchaseType.Prisoners => TryCurrencyPurchase(purchaseId),
            PurchaseType.RewardedVideo => TryRewardedVideoPurchase(purchaseId),
            PurchaseType.PassLevelReward => Observable.Return(PurchaseResult.Unavailable),
        ...
        private IObservable<PurchaseResult> TryRewardedVideoPurchase(string purchaseId) => rewardedVideoPurchaseUseCase
            ...
            .Select(result =>
                result == ShowRewardedVideoResult.Purchased ? PurchaseResult.Success : PurchaseResult.Unavailable
            );
